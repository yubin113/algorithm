...
public class Main {
    ...
    static class Node{
        ...
        public Node(int x, int y, int sum, int breakNum){
            this.x = x;
            this.y = y;
            this.sum = sum;
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if(arr[nextX][nextY] == 0){
                    if(!check[nextX][nextY][node.breakNum]){
                        ...
                        queue.add(new Node(nextX, nextY, node.sum + 1, node.breakNum));
                    }
                }else{
                    if(node.breakNum < K && !check[nextX][nextY][node.breakNum + 1]){
                        check[nextX][nextY][node.breakNum + 1] = true;
                        queue.add(new Node(nextX, nextY, node.sum + 1, node.breakNum +1));
                    }
                }
            }
        }
